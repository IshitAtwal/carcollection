...
const Addsubcars = () => {
      ...
      const submit = async () => {
        ...
        try {
          const result = await fetch("http://localhost:8000/addsubcars", {
            method: "post",
            body: formdata,
          })
...
useEffect(()=>{
    ...
    const showsub=async()=>{
        try{
            ...
            const result=await fetch("http://localhost:8000/showsubcar",{
            method:"get",
            headers:{
                "Content-type":"application/json; charset=UTF-8"
            }
    ...
    <div>
      ...
      <div className="form-container">
        ...
        <div className="form-group">
          ...
          <select class="form-control" onChange={(e) => setcarid(e.target.value)} >
          <option>Select Category</option>
          {
            show.map((d)=>
            <option value={d._id}>{d.brand}</option>
            )
          }
          </select>
        </div>
